...
async function retrieveKBDocs(query, knowledgeBase, knowledgeBaseID) {
  ...
  try {
    ...
    // Define the folder path in your S3 bucket (replace with your actual folder name)
    const folderPath = "CCG NOFO/";  // e.g., "my-folder/"
    ...
    const confidenceFilteredResults = response.retrievalResults.filter(item =>
      item.score > 0.5 && item.location.s3Location.uri.includes(folderPath)
    );
    ...
    const documentUris = confidenceFilteredResults.map(item => {
      return {
        title: item.location.s3Location.uri.slice((item.location.s3Location.uri).lastIndexOf("/") + 1) + " (Bedrock Knowledge Base)",
